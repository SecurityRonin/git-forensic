# Contributing to git-forensic

Thanks for your interest in improving `git-forensic`. This crate parses
untrusted Git object stores from potentially compromised systems, so correctness
and robustness are not negotiable. The bar is high and the workflow is strict —
...
`.git` directories and an independent oracle (`git` itself), not only synthetic
fixtures.
...
- **Fuzzing** — changes to a parser must keep its fuzz target green. Run the
  ...
  cargo +nightly fuzz run delta        # or: loose, commit, tree
  ```
...
- No panics on malicious input — validate every length, offset, and delta
  instruction against both the declared size and the actual buffer; use checked
  or saturating arithmetic.
- Fail loud — surface malformed input as a typed `GitError` with enough context
  to diagnose it. Never swallow an error or substitute a silent default.
